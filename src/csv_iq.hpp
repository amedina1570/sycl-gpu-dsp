// Fast, dependency-free CSV line parser for "<int>,<int>" I/Q pairs, used
// by csv2sigmf.cpp to convert vendor CSV IQ dumps (one signed-integer
// sample pair per line, no quoting/escaping) to raw SigMF at the speed a
// multi-hundred-million-line file demands -- std::getline + std::stringstream
// per line would take far too long at that scale.
#pragma once

namespace csv {

// Parses one line "<int>,<int>" starting at `begin`. Whitespace around
// either number is skipped. The line may end in "\n", "\r\n", or simply at
// `end` (the true end of the file often has no trailing newline).
//
// Returns a pointer just past the parsed line (past the newline, if any),
...
// buffer (so intermediate lines always terminate within [begin, end) and
// never spuriously hit the end-of-buffer case), or (b) the true end of the
// whole input, for the final --  possibly newline-less -- line.
