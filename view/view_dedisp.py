...
# dedisp.cpp writes a <ded_path>.json sidecar alongside its .bin output with
# the exact NFFT/HOP/FS/FC/DM it ran with, so this doesn't carry its own
# stale copy of crab_example.hpp's values. Fall back to the Crab defaults
# for outputs written before that sidecar existed.
meta_path = os.path.splitext(ded_path)[0] + '.json'
if os.path.exists(meta_path):
    with open(meta_path) as fh:
        meta = json.load(fh)
    NFFT, HOP, FS, FC, DM = meta['nfft'], meta['hop'], meta['fs'], meta['fc'], meta['dm']
else:
    NFFT, HOP, FS, FC, DM = 8192, 2048, 20e6, 410e6, 56.7
...
a1.set_ylabel('Frequency (MHz)'); a1.set_title(f'Dedispersed (DM={DM:.1f})')
